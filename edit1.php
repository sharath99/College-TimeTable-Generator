       ...
       foreach ($days as $x)  
        {
            foreach($times as $y)
            {
                ...
                $conn->query($sql);
                $sql2="update alloctable set fname=(select fname from fopt where year='$year' and sem='$sem' and $class='$class' and sub='$_POST[$xid]') where year='$year' and sem='$sem' and class='$class' and day='$x' and time='$y'";
                $conn->query($sql2);
                $xid++;
            }
        }  
